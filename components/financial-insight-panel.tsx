...
export default function FinancialInsightPanel({
  ...
  return (
    <div className="space-y-4">
      <div
        className={`rounded-2xl border p-4 ${verdictClass(
          insight.verdictCode
        )}`}
      >
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-[11px] font-black tracking-[0.2em] opacity-75">
            FINANCIAL VERDICT
          </span>
      ...
      {!compact ? (
        <div className="rounded-2xl border border-violet-400/20 bg-violet-500/10 p-4">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <span className="rounded-full border border-violet-300/20 bg-black/20 px-3 py-1 text-[10px] font-black tracking-[0.14em] text-violet-200">
              KT COMMENT v2
            </span>
          </div>
    ...
    <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
      ...
      <ul className="mt-3 space-y-2">
        {items.map((item) => (
          <li
            key={item.label}
            className={`rounded-xl border px-3 py-2 text-sm font-bold ${toneClass(
              item.tone
            )}`}
          >
            {item.label}
