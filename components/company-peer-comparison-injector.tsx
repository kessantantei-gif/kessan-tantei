...
export default function CompanyPeerComparisonInjector() {
  ...
  useEffect(() => {
    ...
    const run = () => {
      if (document.querySelector("[data-company-peer-comparison='true']")) return;
      const target = findInsertTarget();
      if (!target || !target.parentElement) return;

      const mount = document.createElement("div");
      mount.dataset.companyPeerComparison = "true";
      target.insertAdjacentElement("afterend", mount);
      createRoot(mount).render(<CompanyPeerComparison ticker={ticker} />);
    };
