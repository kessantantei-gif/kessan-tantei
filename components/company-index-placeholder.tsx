    ...
    <main className="min-h-screen bg-[#050816] px-4 py-10 text-white sm:px-8 sm:py-16">
      <article className="mx-auto max-w-5xl">
        <nav className="text-sm text-slate-400" aria-label="パンくず">
          ...
          <Link href={marketHref(marketSegment)} className="hover:text-white">
            {market}
          </Link>
          <span className="mx-2">/</span>
        ...
        <header className="mt-6 rounded-3xl border border-white/10 bg-white/5 p-6 sm:p-10">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="rounded-full border border-yellow-300/20 bg-yellow-400/10 px-3 py-1 text-xs font-black text-yellow-100">
              財務判定準備中
            </span>
          </div>
          <h1 className="mt-3 text-3xl font-black sm:text-5xl">{companyName}</h1>
          <p className="mt-4 max-w-3xl leading-8 text-slate-300">
            証券コード {ticker}。上場市場・業種・取得済みの公式開示を表示しています。
            Financial ScoreとDanger Scoreは、必要な財務データの取得・検証完了後に同じURLへ追加されます。
          </p>
          <div className="mt-5 flex flex-wrap gap-2 text-sm font-bold">
            <span className="rounded-full bg-white/10 px-4 py-2">証券コード {ticker}</span>
            <span className="rounded-full bg-green-500/10 px-4 py-2 text-green-200">
              {market}
            </span>
        ...
        <section className="mt-6 rounded-3xl border border-white/10 bg-white/5 p-6 sm:p-8">
          <h2 className="text-2xl font-black">企業基本情報</h2>
          <dl className="mt-6 grid gap-4 sm:grid-cols-3">
            <Info label="証券コード" value={ticker} />
            <Info label="上場市場" value={market} />
        ...
        <section className="mt-6 rounded-3xl border border-white/10 bg-white/5 p-6 sm:p-8">
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-black">直近の公式開示</h2>
            </div>
            <Link
          ...
          {disclosures.length > 0 ? (
            <ul className="mt-6 space-y-3">
              {disclosures.map((disclosure, index) => {
                ...
                return (
                  <li
                    key={`${disclosure.disclosed_at ?? "unknown"}-${index}`}
                    className="rounded-2xl border border-white/10 bg-black/20 p-5"
                  >
                    <div className="flex flex-wrap gap-2 text-xs font-bold text-slate-400">
                      {date ? <span>{date}</span> : null}
                      {disclosure.document_type ? (
                        <span>{disclosure.document_type}</span>
                      ) : null}
                    </div>
                    {href ? (
                      <a
            ...
            <p className="mt-6 rounded-2xl border border-white/10 bg-black/20 p-5 leading-7 text-slate-300">
              取得済みの開示資料はありません。企業基本情報は引き続き確認できます。
            </p>
          )}
        </section>

        <section className="mt-6 rounded-3xl border border-white/10 bg-white/5 p-6 sm:p-8">
          <h2 className="text-2xl font-black">財務判定で使用する項目</h2>
          <p className="mt-4 leading-8 text-slate-300">
            有価証券報告書、半期報告書、決算短信などの公式開示から取得した数値を、全銘柄共通のルールで整理します。
          </p>
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            <div className="rounded-2xl border border-green-400/15 bg-green-500/5 p-5">
              <h3 className="font-black text-green-200">Financial Signals</h3>
              <p className="mt-2 text-sm leading-7 text-slate-300">
                売上成長率、営業利益率、営業CF、自己資本比率、四半期変化。
              </p>
            </div>
            <div className="rounded-2xl border border-yellow-300/15 bg-yellow-500/5 p-5">
              <h3 className="font-black text-yellow-100">Risk Signals</h3>
              <p className="mt-2 text-sm leading-7 text-slate-300">
                資金繰り、継続企業注記、希薄化、営業CF悪化などの警戒項目。
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="mt-6 grid gap-4 sm:grid-cols-2">
          <Link
            href={marketHref(marketSegment)}
            className="rounded-3xl border border-green-400/20 bg-green-500/10 p-6 transition hover:-translate-y-0.5"
          >
          ...
          <Link
            href="/ranking"
            className="rounded-3xl border border-cyan-400/20 bg-cyan-500/10 p-6 transition hover:-translate-y-0.5"
          >
            <p className="text-xs font-black tracking-[0.2em] text-cyan-300">RANKING</p>
            <h2 className="mt-2 text-xl font-black">決算ランキングを見る</h2>
          </Link>
        </section>
      </article>
    </main>
...
function Info({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-black/20 p-5">
      <dt className="text-xs font-bold tracking-[0.16em] text-slate-400">{label}</dt>
