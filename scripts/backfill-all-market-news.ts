...
async function main() {
  ...
  let offset = process.argv.includes("--restart") ? 0 : await readOffset(companies.length);

  if (process.argv.includes("--restart")) await saveOffset(0);

  console.log("=== 全市場ニュース初回取得 ===");
  console.log(`対象会社: ${companies.length}`);
  ...
  while (offset < companies.length) {
    ...
    await saveOffset(offset >= companies.length ? 0 : offset);
...
main().catch((error) => {
  console.error("全市場ニュース初回取得に失敗しました。", error);
  process.exit(1);
});
